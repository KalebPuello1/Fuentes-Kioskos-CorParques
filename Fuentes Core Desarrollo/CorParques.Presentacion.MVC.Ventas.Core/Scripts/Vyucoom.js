...
$("document").ready(() => {
           ...
           /* if ($("#selectMedioPago option:selected").text() == " --Seleccione Forma Pago-- ") {
            ...
            if ($("#IdApagar").val().length >= 1){
                ...
                if (ingFunction == 0) {
                    ...
                    //EjecutarAjax(`${ruta}/InsertDatos`, "GET", { dato: CodPedido.val(), cliente: Cliente.val() == "22222-SIN REGISTROS CLIENTE" ? identidadCliente : Cliente.val(), mpago: $("#selectMedioPago option:selected").text(), pago: datoEnviarPagar, cambioo: da, medioPago: listaDetallePago, exist: clienteNoExiste}, "lol", null)
                    EjecutarAjax(`${ruta}/InsertDatos`, "GET", { dato: CodPedido.val(), cliente: Cliente.val() == "22222-SIN REGISTROS CLIENTE" ? identidadCliente : Cliente.val(), mpago: '', pago: valorPagadoUsuario, cambioo: da, medioPago: listaDetallePago, exist: clienteNoExiste, aPagar : datoEnviarPagar}, "lol", null)
    ...
    $("#agregarCliente").click((e) => {
        //if (nuevoCliente.value != "" && IdenCliente.value != "") {
        if (IdenCliente.value != "") {
            ...
            identidadCliente = identClientee + "-" + "Cliente no existe"
            //
                        ...
                        success: (e) => {
                            ...
                            if (e.Valor == "Pedido_no_existe") {
                                ...
                                if (e.NombCliente == "22222-SIN REGISTROS CLIENTE" && nuevoClientee == "") {
                                    ...
                                    nuevCliente.style.display = "block"
                                    //
                                    //document.getElementById("ocultaruno").style.display = 'block';
                                    ...
                                    document.getElementById("ocultaruno").style.display = 'none';
...
*/
                                ...
                                //nuevCliente.style.display = "none"
                                console.log("entro el segundo if")
                                //document.getElementById("ocultaruno").style.display = 'none';
                                document.getElementById("ocultardos").style.display = 'none';
                                $("#IdApagar").mask("000.000.000", { reverse: true });
                                nuevCliente.style.display = "block"
                                ...
                                if (e.Valor.length == 6) {
                                    ...
                                    pagarUsuario.val(`${e.Valor}`)

                                    //ValorCostoComparar = e.Valor;
                                    ValorCostoComparar = $("#IdApagar").val()
                                    console.log(" ffff " + ValorCostoComparar)
...
function AgregarPagoo() {
    ...
    console.log($("#inputValorPago").val() + " ----------------> _________________________" +  $("#inputValorPago").val() )
            ...
            if (($("#inputReferencia").val() == "" && IdMedioPago == parametros.IdMedioPagoEfectivo)
                ...
                if (!hayPagoEfectivoAdd) {
                    if (transaccionRedebanHabilitadaa == "1" && IdMedioPago == "2" && resultadoTransaccionRedebanExitosa == true) {
                        ...
                        console.log("3 ******  paso por medio de pago ************");
                        console.log("3");
                        var ObjectPago = new Object();
                        ObjectPago.IdMedioPago = parseInt($("#selectMedioPago").val());
...
//                enviarDato(Pago.val(), CodPedido.val())
                ...
                ///Esto e donde debo validar para pasar el dato nuevo para poder cambiar la interfas
                //if (nuevoCliente.value != "") {
                if (IdenCliente.value != "") {
                    console.log(Pago.val())
                    //pagoUsuario += parseFloat(Pago.val())
                    pagoUsuario += parseInt(Pago.val().replace(".", "").replace(".", ""))
                    //enviarDatoo(Pago.val())
                    enviarDatoo(pagoUsuario)
                } else { 
                    console.log(Pago.val(), CodPedido.val())
                    //enviarDato(Pago.val(), CodPedido.val())
                    //pagoUsuario += parseFloat(Pago.val())
                    pagoUsuario += parseInt(Pago.val().replace(".", "").replace(".", ""))
                    //enviarDato(Pago.val(), $("#IdApagar").val())
                    enviarDato(pagoUsuario, $("#IdApagar").val())
                }
       ...
       // $("#inputValorPago").val("")
        ...
        /* var franquiciaTarjeta = "";
         ...
         var numeroRecibo = ""; */
        var IdMedioPago = $("#selectMedioPago").val();
        var montoPagoCliente = $("#inputValorPago").val();
    $("#inputValorPago").val("");
    $("#selectMedioPago").val("0")
    $("#selectMedioPago").change()
    /////////////////////////////////////////////////////////// 18/04/2022
    ...
    MostrarMensaje("Importante","Se agrego el abono correctamente", "success")
}
        ...
        if (clienteNoExiste != 1)
        {
            ...
            if (pedidoNoExiste != 1) {
                if (cambio < 0) {
                    ...
                    console.log(sum + "  ---- " + total + " QQQdddd " + cambio)
                    $("#cambio").html("<p style='color:red'> Cambio: $" + EnMascarar(cambio) + "</p>");
                    $("#total").html("<p>Total abonado<h1 style='font-weight: bold;'> $" + EnMascarar(sum) + "</h1></p>");
                } else {
                    $("#cambio").html("Cambio: $" + EnMascarar(0));
                ...
                //$("#total").html("<p>Total abonado<h1 style='font-weight: bold;'> $" + EnMascarar(cambio) + "</h1></p>");
                //$("#total").html("<p>Total abonado<h1 style='font-weight: bold;'> $" + EnMascarar(ValorCostoComparar) + "</h1></p>");
                console.log(sum + "  ---- " + total + " QQQf " + ValorCostoComparar)
...
function ObtenerTotal() {
    ...
    $.each(lstProductosCompra, function (i, item) {
        ...
        console.log(listaDetallePago)
        console.log(lstProductosCompra + " listas --------------------> fffffff lol")
        console.log(total)
    ...
    console.log(impuestoTotal + " compra  --------------------> lol antes ")
...
function EnMascarar(valor) {
    console.log(valor + " compra  --------------------> enmascarar lol")
    var val = $.trim(valor);
    var fn = val;
    console.log(val)
    console.log(valor)
...
const enviarDato = (pago, pagar) => {
    console.log("**************************************** estes es igual " + pago + " *************** " + CodPedido)
    var total = 0;
    var sHTML = '';
    ...
    ValorCostoComparar = pagarr.val().replace(".", "").replace(".", "")

    console.log("Por aca pasa el dato cuando no tiene el cliente con los pagos ----> " + pago + " validacion : 2" + CodPedido)
    ...
    console.log(pago + " _______ " + CodPedido)
    console.log(datoEnviarPagar)
    ...
    $.ajax({
        ...
        success: (e) => {
            ...
            da = e.diferencia
            valorPagadoUsuario = e.pago
            console.log(da + " -------------------------------------> dato diferencia" + e.diferencia + " ************************** " + e.pago + "  ************** " + e.pagar)
...
const enviarDatoo = (pago) => {
    ...
    console.log("Por aca pasa el dato cuando no tiene el cliente con los pagos ----> " + pago + " validacion : 1")
