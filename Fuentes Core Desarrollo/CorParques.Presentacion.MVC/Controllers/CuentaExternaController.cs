...
{
    public class CuentaExternaController : ControladorBase
    {
        [AllowAnonymous]
        [HttpGet]
        [Route("api/LoginCodigoFechaAbiertaExternos/")]
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> LoginExterna(string user1, string pwd)
        {
            ViewBag.Validar = false;

            pwd = Encripcion.Encriptar(pwd, ConfigurationManager.AppSettings["llaveEncripcion"]);

            var user = await GetAsync<Usuario>($"Usuario/GetByUserPwdExterno?pwd={Server.UrlEncode(pwd)}&user={user1}&Punto=0");
            ...
            if (user != null)
            {
                ...
                {
                    return Json(new RespuestaViewModel { Correcto = true, Mensaje = "OK", Elemento = Url.Action("Index", "CodigoFechaAbiertaExternos") }, JsonRequestBehavior.AllowGet);
                }
        ...
        [AllowAnonymous]
        public ActionResult ActualizarCambioClave(UsuarioExterno modelo)
        {
            //var user = (Usuario)Session["UsuarioAutenticado"];
            var user = (Usuario)Session["UsuarioAutenticado"];
            ...
            if (pdw == user.Password)
            {
                if (modelo.Password != modelo.Password2)
                {
                    if (modelo.Password2 == modelo.ConfirmPassword2)
                    {
                        ...
                        //
                        if (PutAsync<Usuario, string>("Usuario/ActualizarUsuarioExterno", user) != null)
                            //BuscarCodigos -- 
                            return Json(new RespuestaViewModel { Correcto = true, Mensaje = "Su contraseña se ha cambiado exitosamente.", Elemento=Url.Action("Index", "CodigoFechaAbiertaExternos") }, JsonRequestBehavior.AllowGet);
