...
$(function () {
    ...
    $("#btnConsultarRest").click(function () {
        ...
        objeto.FechaInicialP = fechaInicial;
        objeto.FechaFinalP = fechaFinal;
        objeto.IdTipoPedido = isNaN(IdTipoPedido) ? 0 : parseInt(IdTipoPedido);
        objeto.CodCliente = IdCliente;
        objeto.codPedido = codPedido;
        objeto.CodVendedor = IdResponsable;


        if (validarFormulario("listView")) {
            EjecutarAjax(urlBase + "ReporteBonoRegalo/GenerarArchivo", "GET", objeto, "cargarTablaRest", null);
        }
    });
    $("#btnLimpiarRest").click(function () {
        window.location = urlBase + "ReporteBonnoRegalo";
    });
...
function cargarTabla(datos, params) {
    if (datos.length > 0) {
        if (datos.indexOf("Error") >= 0) {
            ...
            window.location = urlBase + 'ReporteBonoRegalo/Download?Data=' + datos;
