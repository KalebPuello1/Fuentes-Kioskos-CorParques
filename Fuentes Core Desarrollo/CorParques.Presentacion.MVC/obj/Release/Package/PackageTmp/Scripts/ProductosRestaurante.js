...
var lstProductosAcompanamientosAdmin = [];
var _IdProductoAdmin = 0;
var _BanderaBoton = 0;
var lstDetalleProdAdmin = [];

$(function () {
   
    $("#lnkAddProductosAraza").click(function () {
        _BanderaBoton = 4;
        EjecutarAjax(urlBase + "PedidoA/GetPartialProductos", "GET", null, "printPartialModal", {
            title: "Lista Productos", modalLarge: true
        });
    });
    ...
    $("#btnSaveGeneric").click(function () {
        if (_BanderaBoton === 0) {
            successGuardarAcompaProduAdmin();
        }
        if (_BanderaBoton === 1) {
            successGuardarDatosProdAdmin();
        }
        if (_BanderaBoton === 2) {
            successGuardarDatosTipoAcompAdmin();
        }
        if (_BanderaBoton === 3) {
            successGuardarDatosTipoAcompAdmin();
        }
        if (_BanderaBoton === 4) {
            successGuardarProductosAdmin();
        }
      
    });

    $("#lnkAddTipoAcompa").click(function () {
        _BanderaBoton = 3;
    
        EjecutarAjax(urlBase + "PedidoA/GetPartialTipoAcompa", "GET", null, "printPartialModalAdmin", { title: "Crear tipo acompañamiento" });
    });
    $(".lnkDeleteTipoAcompa").click(function () {
        MostrarConfirm("Importante", "¿Está seguro que desea eliminar este tipo de acompañamiento?", "DeleteTipoAcompa", $(this).data("id"));
        //if (confirm("Está seguro que desea eliminar este usuario?"))
        //    EjecutarAjax(urlBase + "Usuarios/Delete", "GET", { id: $(this).data("id") }, "successDelete", null);
    });
    $(".lnkEditProduRestaurante").click(function () {
        _IdProductoAdmin = $(this).data("id");
        _BanderaBoton = 1;
        EjecutarAjax(urlBase + "PedidoA/ObtenerProdRestaurante", "GET", { id: $(this).data("id") }, "printPartialModal", { title: "Editar producto" });
    });
    $(".lnkGetAcompa").click(function () {
        _BanderaBoton = 0;
        lstProductosAcompanamientosAdmin = [];
        _IdProductoAdmin = $(this).data("id");
        EjecutarAjax(urlBase + "PedidoA/ObtenerAcompaTipo", "GET", { id: $(this).data("id") }, "printPartialModalAdmin", { title: "Editar producto", func: "successGuardarAcompaProduAdmin" });
    });
    $(".lnkEditTipoAcompaAdmin").click(function () {
        _IdProductoAdmin = $(this).data("id");
        _BanderaBoton = 2;
        EjecutarAjax(urlBase + "PedidoA/GetByIdTipoAcompa", "GET", { idTipoAcompa: $(this).data("id") }, "printPartialModal", { title: "Editar tipo acompañamiento" });
    });
    $(".lnkAgregarAcompaProd").click(function () {
        var idcargar = $(this).data("id");
        var bandera = 0;
        $.each(lstProductosAcompanamientosAdmin, function (i) {
            if (lstProductosAcompanamientosAdmin[i].IdAcomp == idcargar) {
        ...
        if (bandera == 0) {
            ...
            var myObj = {};
            myObj.IdProducto = $(this).data("id");
            myObj.IdAcomp = $(this).data("id");
            myObj.NombreTipo = $(this).attr("id");
            myObj.Nombre = $(this).attr("name");

            lstProductosAcompanamientosAdmin.push(myObj);
            ActualizarTaablaAcompaAdmin();
        }
    });
    $(".gridacompinicial").click(function () {

        MostrarConfirm("Importante", "¿Está seguro que desea eliminar este producto?", "DeleteProductoAcompanamiento", $(this).data("id"));
        //if (confirm("Está seguro que desea eliminar este usuario?"))
        //    EjecutarAjax(urlBase + "Usuarios/Delete", "GET", { id: $(this).data("id") }, "successDelete", null);
    });
    $(this).data("codigoventa");
...
function DeleteTipoAcompa(Id) {
    EjecutarAjax(urlBase + "PedidoA/DeleteTipoAcompa", "GET", { idTipoAcompa: Id }, "GuardarTipoAcompAdmin22", null);
}
function successGuardarAcompaProduAdmin() {
    cerrarModal('modalCRUD');

    EjecutarAjaxJson(urlBase + "PedidoA/GuardarAcompaProduAdmin", "POST", {
        lstProductosAcompanamientosAdmin: lstProductosAcompanamientosAdmin,
        IdProducto: _IdProductoAdmin
    }, "GuardarAcompaProduAdmin22");
}
function successGuardarDatosProdAdmin() {
    cerrarModal('modalCRUD');
    var myObj = {};
    myObj.IdProducto = _IdProductoAdmin;
    myObj.Id_TipoAcomp = $("#Id_TipoAcomp").val();
    myObj.Id_TipoProdRestaurante = $("#Id_TipoProdRestaurante").val();


    EjecutarAjaxJson(urlBase + "PedidoA/ActualizarProductoAdminRestaurante", "POST", {
        model: myObj
    }, "GuardarAcompaProduAdmin22");
}
function successGuardarProductosAdmin() {
    var listProductosAgregar = [];
    $('.checkboxProd:checked').each(function () {
       
        var myObj = {};
        var nombre1 = "#IdTipAcompa-" + $(this).val();
        var nombre2 = "#IdTipProduR-" + $(this).val();
        myObj.IdProducto = $(this).val();
        myObj.Id_TipoAcomp = $(nombre1).val();
        myObj.Id_TipoProdRestaurante = $(nombre2).val();
        listProductosAgregar.push(myObj);
    });


    EjecutarAjaxJson(urlBase + "PedidoA/AgregarProducAraza", "POST", {
        listaProductosAgregar: listProductosAgregar
    }, "GuardarAcompaProduAdmin22");
}

function GuardarAcompaProduAdmin22(rta) {
    ...
    //return false;
    window.location = urlBase + "AdminRestaurante/Productos";


}

function successGuardarDatosTipoAcompAdmin() {
    cerrarModal('modalCRUD');
    var myObj = {};
    myObj.IdTipoAcomp = $("#IdTipoAcomp").val();;
    myObj.Nombre = $("#Nombre").val();
    myObj.Estado = $("#Estado").val();


    EjecutarAjaxJson(urlBase + "PedidoA/UpdateTipoAcompa", "POST", {
        modelo: myObj
    }, "GuardarTipoAcompAdmin22");
}

function GuardarTipoAcompAdmin22(rta) {

    /* if (rta == "NO") {*/
    //MostrarMensajeRedireccion("Importante", "No se puede cerrar la mesa porque no se ha cancelado el pedido en caja", null, "warning");
    //$("#btnCerrarMesa").attr("disabled", false);
    //return false;
    window.location = urlBase + "AdminRestaurante/TipoAcompanamiento";


}

function DeleteProductoAcompanamiento(Id) {


    if (lstProductosAcompanamientosAdmin.length == 1) {
        lstProductosAcompanamientosAdmin = [];
    }
    else {

        var lstFinalAcompa2 = lstProductosAcompanamientosAdmin;

        $.each(lstProductosAcompanamientosAdmin, function (i) {
            if (lstProductosAcompanamientosAdmin[i].IdProducto == Id) {
                lstProductosAcompanamientosAdmin.splice(i, 1);
                return false
            }
        });
    }
...
function printPartialModalAdmin(data, obj) {
    ...
    $.each($(".gridacompinicial"), function (i, item) {

        var myObj = {};
        myObj.IdProducto = $(item).data("id");
        myObj.IdAcomp = $(item).data("id");
        myObj.NombreTipo = $(item).attr("id");
        myObj.Nombre = $(item).attr("name");
...
function ActualizarTaablaAcompaAdmin() {
    EjecutarAjaxJson(urlBase + "PedidoA/ActualizarTablaAompa", "POST", {
        ListaAcompa: lstProductosAcompanamientosAdmin,
    }, "successActualizarAcomSele", { div: "#listViewProdSele" });
}
function successActualizarAcomSele(data, values) {
    if (isNaN(data)) {
...
function AgregarProductoACompra(IdProducto, tmpList) {
    ...
    if (tmpList != null) {
        ...
        acompa = "prueba";
        EjecutarAjax(urlBase + "PedidoA/Acompa", "GET", { IdProducto: IdProducto }, "printPartialModalPedidosA", {
