...
function InicializarAM() {

    InhabilitarCopiarPegarCortar("txt_CodigoBarrasAM");

    if (ValidacionOperativaPunto()) {
        $("#txt_CodigoBarrasAM").keyup(function () {
            if (inicializadointerval) {
                inicializadointerval = true;
                var refreshIntervalId = setInterval(function () { ValidarCodigoDeBarrasAM(); inicializadointerval = false; clearInterval(refreshIntervalId); }, 300);
            }
        });

        $('#txt_CodigoBarrasAM').focusout(function () {
            setTimeout(function () { $('#txt_CodigoBarrasAM').focus() }, 500);
        });
...
///Valida si el codigo de barras tiene acceso a la atraccion.
    ...
    if (strCodigoBarras != '') {
        ...
        if (ValidarSiExisteCupoDebito(strCodigoBarras)) {
            if (lngValorAcumulado > 0) {
                ...
                EjecutarAjax(urlBase + "Atraccion/TransferirSaldo", "GET", { CodigoACargar: strCodigoBarras, CodigoDeBarrasDescargar: RetornarCodigosDeBarraDescargar(strCodigoBarras), ValorAcumulado: lngValorAcumulado }, "RespuestaTranferencia", null);
            }
            LimpiarCuposDebito();
        } else {
            if (!AnularIngresoCupoDebito(strCodigoBarras)) {
                EjecutarAjax(urlBase + "Atraccion/ValidarAccesoAtraccion", "GET", { CodigoDeBarras: strCodigoBarras, ValorAcumulado: lngValorAcumulado, ValorAcumuladoConvenio: lngValorConvenio, CodigoDeBarrasDescargar: RetornarCodigosDeBarraDescargar(strCodigoBarras) }, "RespuestaValidacion", null);
            }
        }
    } else {
        //Limpia cupo debito.
    ...
    if (blnValido) {
        ...
        if (strCodigoMensaje == 'CB0') {
            ...
            if (strCodigoMensaje == 'CD1') {
                ...
                //Es cupo debito. No ingresa por saldo insuficiente.
...
function ValidarCodigoDeBarrasAM() {

    var strCodigoBarras = "";
    var lngValorAcumulado = 0;
    var lngValorConvenio = 0;

    strCodigoBarras = $("#txt_CodigoBarrasAM").val();
    if (strCodigoBarras != '') {
        lngValorAcumulado = ObtenerAcumuladoCupoDebito(strCodigoBarras);
        lngValorConvenio = ObtenerAcumuladoConvenio(strCodigoBarras);
        if (ValidarSiExisteCupoDebito(strCodigoBarras)) {
    ...
    $("#txt_CodigoBarrasAM").val("");
}


$("#BtnIngresarManualAM").click(function () {

    $("#Text1").hide();
    $("#FrmAtraccionManual").show();
    //Deshabilitar función antigua
    $("#txt_CodigoBarras").hide();

});


$("#BtnAceptarAM").click(function () {

    var u = document.getElementById("txt_CodigoBarrasAM").value;

    if (u == "") {
        alert("¡ CAMPO N° DE ATRACCIÓN VACIO !");
    } else {
        ValidarCodigoDeBarrasAM()
    }
...
$("#txt_CodigoBarrasAM").keypress(function (event) {
    if (event.which == 13) {
        ValidarCodigoDeBarrasAM()
