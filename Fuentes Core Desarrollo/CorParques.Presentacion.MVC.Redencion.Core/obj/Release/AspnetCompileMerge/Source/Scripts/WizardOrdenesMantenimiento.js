...
(function ($) {
    'use strict';
    localStorage.HoraInicio = createDateObj(new Date());
    localStorage.HoraInicioOrden = createDateObj(new Date());
    $.fn.modalSteps = function (options) {
        ...
        $modal
            .on('show.bs.modal', function () {
                ...
                $actualStep = $('<input>').attr({
                    'type': 'hidden',
                    'id': 'actual-step',
                    'value': parseInt($("#inicio").val()),
                });

                $modal.find('#actual-step').remove();
                $modal.append($actualStep);

                if (parseInt($("#inicio").val()) == parseInt($("#totalPasos").val())) {
                    actualStep = parseInt($("#inicio").val());
                    nextStep = actualStep;
                    $btnNext
                    .attr('data-step', 'complete')
                    .html(settings.btnLastStepHtml);
                    $btnNext = $modal.find('.js-btn-step[data-orientation=""]')
                }
                else {
                    actualStep = parseInt($("#inicio").val());
        ...
        $modal.find('.js-btn-step').on('click', function () {
            ...
            steps = parseInt($("#totalPasos").val());
            // Callback on Complete
            if ($btn.attr('data-step') === 'complete') {
                settings.completeCallback();
                steps = parseInt($("#totalPasos").val()) - 1;
                $("#aprobado").prop('checked', false);
                if (!Insertar($actualStep.val())) { return; }
                else {
            ...
            if (orientation === 'next') {
                if (!GuardarRegistro($actualStep.val())) { return; }
                $btnNext.removeAttr('data-dismiss');
                nextStep = actualStep + 1;
                localStorage.HoraInicio = createDateObj(new Date());
