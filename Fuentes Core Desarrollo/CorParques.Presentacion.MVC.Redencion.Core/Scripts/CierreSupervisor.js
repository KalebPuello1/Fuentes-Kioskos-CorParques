...
$(function () {
    ...
    $("#IdUsuarioCreacion").change(function () {
        ConsultarRecoleccion($(this).val());
    });
...
function ConsultarRecoleccion(IdTaquillero) {
    if (IdTaquillero != "") {
        EjecutarAjax(urlBase + "CierreSupervisor/BuscarRecoleccion", "GET", { IdTaquillero: parseInt(IdTaquillero) }, "CargarPagina", null);        
    } else {
...
//Muestra la ventana de login para introducir la contraseña del taquillero.
function MostrarLogin()
{    
    CerrarResumen();
    EjecutarAjax(urlBase + "CierreSupervisor/ObtenerLogin", "GET", null, "printPartialModal", {title: "Confirmación taquillero", hidesave: true, modalLarge: false});
}
...
function Login(password, observaciones) {
    ...
    IdUsuario = $("#IdUsuarioCreacion").val();
...
function respuestaLogin(data) {
    ...
    urlaction = urlBase + "CierreSupervisor/GuardarObservaciones";
    if (data.Correcto) {
...
{
    $("#IdUsuarioCreacion").attr("disabled", "disabled");
    $(".actionBar").hide();
}
