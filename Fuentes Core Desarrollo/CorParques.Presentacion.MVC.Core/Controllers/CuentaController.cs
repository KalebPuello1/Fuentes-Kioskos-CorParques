...
namespace CorParques.Presentacion.MVC.Core.Controllers
{
    public class CuentaController : ControladorBase
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> Login(string user1)
        {
        ...
        private async Task<ActionResult> ValidaIngreso(Usuario user, bool validadpwd2)
        {
            ViewBag.Validar = false;
            var idPuntoConfigurado = IdPunto;
            ...
            if (_isPuntoValid)
            {
                ...
                if (validadpwd2 && (_IdTipoPunto != (int)Enumerador.TiposPuntos.Atraccion) && (_IdTipoPunto != (int)Enumerador.TiposPuntos.Parqueadero))
                {
                    Session["UsuarioAutenticadoTemp"] = user;
                    return Json(new RespuestaViewModel { Correcto = true });
                }
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> ValidaContrasena2(string Pwd2, string ChangePwd2, string ConfirmPwd2)
        {
