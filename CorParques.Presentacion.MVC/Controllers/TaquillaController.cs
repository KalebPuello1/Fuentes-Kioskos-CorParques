...
{
    public class TaquillaController : ControladorBase
    {
        public async Task<ActionResult> Index()
        {
            var lista = await GetAsync<IEnumerable<Puntos>>($"Puntos/GetAllxTipoPunto/{(int)Enumerador.TiposPuntos.Taquilla}");
            return View(lista);
        }
        ...
        public async Task<ActionResult> GetById(int id)
        {
            ...
            ViewBag.Estados = await GetAsync<IEnumerable<TipoGeneral>>("Estado/ObtenerEstados/7");
            return PartialView("_Edit", item);
        }
        public async Task<ActionResult> Insert(Puntos modelo)
        {
            modelo.UsuarioCreacion = 1;
        ...
        {
            modelo.UsuarioModicifacion = 2;
            var imageOrigin = string.Empty;
            if (!string.IsNullOrEmpty(modelo.Imagen))
            {
                imageOrigin = modelo.Imagen;
                modelo.Imagen = $"{modelo.Id}.png";
            }
            modelo.IdTipoPunto = (int)Enumerador.TiposPuntos.Taquilla;
        ...
        private string moveFile(string name, string newName)
        {
            ...
            var pathDest = Path.Combine(Server.MapPath("~/Images/Parking/"), newFile);
