...
{
    public class ReportePedidoRestauranteController : ControladorBase
    {
        ...
        [HttpGet]
        public async Task<string> GenerarArchivo(ReportePedidoRestaurante modelo)
        {
            ...
            try
            {
                ...
                IEnumerable<ReportePedidoRestaurante> inv =
                ...
                await GetAsync<IEnumerable<ReportePedidoRestaurante>>($"ReportePedidoRestaurante/ObtenerReporteRestaurante/{modelo.FechaInicial ?? "null"}/{modelo.FechaFinal ?? "null"}/{modelo.Id_Vendedor }/{modelo.CodSapAlmacenOrigen ?? "null"}/{modelo.IdZona}/{modelo.Id_Mesa}/{modelo.IdEstadoMesa }");

                if (inv != null)
                {
                    if (inv.Count() > 0)
                        strRetorno = objReportes.GenerarReporteRestaurante(inv);
                }
            }
            catch (Exception ex)
            {
