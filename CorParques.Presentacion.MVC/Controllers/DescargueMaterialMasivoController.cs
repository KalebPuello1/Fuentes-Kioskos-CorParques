    ...
    public class DescargueMaterialMasivoController : ControladorBase
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            if (rta != null)
            {
                ...
                modelo.ListaPedidos = rta.Select(x => new TipoGeneral { Nombre = x.NombreCliente, CodSAP = x.CodigoVenta }).ToList();
                modelo.ListaPedidos = modelo.ListaPedidos.GroupBy(x => x.CodSAP).Select(x => x.First()).ToList();
                ViewBag.listaPedidos = rta;
                ViewBag.Tabla = rta.GroupBy(x => new { x.IdSolicitudBoleteria, x.Nombre, x.CodigoVenta, x.CodigoSap })
                    .Select(m => new ProductosPedidos
                    {
                        Cantidad = m.Sum(x => x.Cantidad),
                        Nombre = m.Key.Nombre,
                        CodigoVenta = m.Key.CodigoVenta,
                        CodigoSap = m.Key.CodigoSap,
                        IdSolicitudBoleteria = m.Key.IdSolicitudBoleteria
                    });
            }
        ...
        //    productos = productosPedido.Select(x => new Producto
        //    {
        //        IdProducto = x.IdProducto,
        //        CodigoSap = x.CodigoSap,
        //        IdDetalleProducto = x.IdBoleteria,
        //        Pedido = x.CodigoVenta,
        //        Posicion = Convert.ToInt32(x.Posicion)
        //    }).ToList();
