        ...
        if (validarFormulario("frmInventarioOperativo")) {
            ...
            $.each($(".Brazaletes"), function (i, item) {
                if ($(item).val().length > 0) {
                    var id = $(item).data("id");
                    var _obj = { "Id": id, "Cantidad": parseInt($(item).val()) };
            ...
            var _inventarioFinal = new Object();
            $.each(_inventario, function (i, item) {
                switch (item.name) {
                    case "IdSupervisor":
                        _inventarioFinal.IdSupervisor = item.value;
                        break;
                    case "hdPuntos":
                        _inventarioFinal.hdPuntos = item.value;
                        break;
                    case "Observacion":
                        _inventarioFinal.Observacion = item.value;
                        break;
                }
            });


            _inventarioFinal.Brazaletes = [];
            _inventarioFinal.Brazaletes = listaBrazalete;

            EjecutarAjax(urlBase + "Reabastecimiento/ObtenerDetalleInventario", "POST", JSON.stringify(_inventarioFinal), "printPartialModal", {
...
$("#frmInventarioOperativoPuntos #IdTaquillero").change(function () {

    var idTaquillero = $(this).val();
    NombreTaquillero = $(this).find('option:selected').text();
    if (idTaquillero.length > 0) {
        RespuestaValidTaquillero(idTaquillero);
    }
...
function Cancelar() {
    window.location = urlBase + "Reabastecimiento/EntregaInventario";
}

function CancelarAlistamientoPunto() {
    window.location = urlBase + "Reabastecimiento/EntregaInventarioPuntos";
}

function RespuestaValidSupervidor(idSupervisor) {
    $(".tdCantSup").html("0")
    $.ajax({
        url: urlBase + 'Reabastecimiento/ObtenerBrazaletesPorSupervisor',
        ...
        success: function (r) {
            ...
            $.each($(".Brazaletes"), function (i, item) {
                ...
                $(this).val("");
                $.each(r, function (j, jitem) {
                    if ($(item).data("id") == jitem.IdBrazalete) {
                        $(item).parent().prev().html(jitem.CantidadFinal);
                    }
                });
            });
        },
...
function RespuestaValidTaquillero(idTaquillero) {
    $(".tdCantSup").html("0")
    $.ajax({
        url: urlBase + 'Reabastecimiento/ObtenerBrazaletesPorTaquillero',
        ContentType: 'application/json; charset-uft8',
        dataType: 'JSON',
        async: false,
        type: 'GET',
        ...
        success: function (r) {
            ...
            isListTaquillero = false;
            $.each($(".Brazaletes"), function (i, item) {
                $(this).attr("readonly", false);
                $(this).val("");
                $.each(r, function (j, jitem) {
                    if ($(item).data("id2") == jitem.IdTipoBrazalete) {
                        $(item).parent().prev().html(jitem.EnCaja);
                    }
                });
            });
        },
...
function Login(password, observaciones) {
    ...
    if ($("#frmInventarioOperativoPuntos").length > 0) {
        ...
        DetalleInventario.IdTaquillero = _idUsuario;
        $.each(DetalleInventario.Apertura, function (i, item) {
...
function respuestaLogin(data) {
    if (data.Correcto) {
        ...
        if ($("#frmInventarioOperativo").length > 0)
            urlaction = urlBase + "Reabastecimiento/GuardarInformacionInventario";
        else if ($("#frmInventarioOperativoPuntos").length > 0)
            urlaction = urlBase + 'Reabastecimiento/ActualizarInformacionInventario';
        //
...
function RespuestaActualizarInventario(data) {
    if (data.Correcto) {
        ...
        if ($("#frmInventarioOperativo").length > 0)
            urlaction = "Reabastecimiento/EntregaInventario";
        else if ($("#frmInventarioOperativoPuntos").length > 0)
...
function ListaDetalle() {
    ...
    EjecutarAjax(urlBase + "Reabastecimiento/ObtenerDetalleInventarioPuntos", "POST", JSON.stringify({ modelo: DetalleInventario }), "printPartialModal", {
        title: "Confirmación supervisor", hidesave: true, modalLarge: false
    });
}
